...
const thermalUrl = 'https://lh3.googleusercontent.com/aida-public/AB6AXuAgxOMkQBM4TfRLL7azZegU-EACze_Vv3LvEidrDzQrESjEBWgGPvn8HQ2dfQ1V3tXxTz6j-pnZg2wb1PTnErB-oPcx1H8wGfCOVyIteZPDrTosnuYr8nNixmw9nqaUhPLbt4Sv5x-I-cCgcXrpZGBktu7d2eVm9FO4QAkdGwwJL7XU7a_naFfJMEHlR1Y0HuNY5kwJLytJy1BlYxohDDBdP4gad0SB3KYPY5C6BxctvZQNGX0213NtNQ'
...
function MissionOverview({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    if (!canvas || !video || !aiActive || cameraStatus !== 'connected') return
  ...
  return <div className="flex h-screen justify-center overflow-hidden bg-[#0b0e14]">
    <div className="relative flex h-full w-full overflow-hidden bg-surface-container-lowest">
      ...
      <main className="relative flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="z-40 flex h-16 w-full shrink-0 items-center justify-between border-b border-white/5 bg-surface px-4 md:px-6"><div className="flex items-center gap-4"><h2 className="font-headline-sm text-lg font-bold text-on-surface">Mission Overview</h2><div className="flex items-center gap-2 rounded-full border border-white/10 bg-surface-container-high px-3 py-1"><Icon className="text-[16px] text-secondary">emergency</Icon><span className="font-data-md text-sm text-secondary">SAR-2026-041</span><span className="ml-1 h-1.5 w-1.5 animate-pulse rounded-full bg-error" /><span className="font-label-caps text-xs text-error">Active</span></div></div><div className="flex items-center gap-3 md:gap-6"><div className="hidden items-center gap-2 font-data-md text-sm text-on-surface-variant md:flex"><Icon className="text-[18px]">schedule</Icon>{time.toUTCString().slice(17, 25)} UTC</div><div className="grid h-8 w-8 place-items-center overflow-hidden rounded-full border border-primary/30 bg-surface-variant"><Icon className="text-on-surface-variant">person</Icon></div></div></header>
        <div className="flex-1 overflow-y-auto p-4 pb-6 md:p-6">
          <div className="alert-pulse glass-panel mb-4 flex flex-wrap items-center justify-between gap-4 rounded-r-lg border-l-4 border-error p-4"><div className="flex items-start gap-4"><Icon className="mt-1 text-[28px] text-error">warning</Icon><div><h3 className="font-headline-sm mb-1 text-lg font-bold text-error">{detections.length ? 'Potential human detected in camera feed' : 'Detection system standing by'}</h3><div className="data-font flex gap-4 text-sm text-on-error-container"><span>PERSON: {detections.length}</span><span>AI: {modelStatus.toUpperCase()}</span></div></div></div><button className="rounded-md bg-error px-6 py-2.5 font-label-caps text-xs text-on-error hover:bg-error/80">Review Detection</button></div>
          <div className="grid min-h-[600px] grid-cols-12 gap-4">
            <div className="col-span-12 flex h-full flex-col gap-4 lg:col-span-8">
              <div className="group relative flex min-h-[400px] flex-1 flex-col overflow-hidden rounded-2xl border border-white/10 bg-black shadow-2xl"><div className="absolute left-4 top-4 z-20 flex items-center gap-2 rounded-md border border-white/10 bg-black/60 px-3 py-1.5 backdrop-blur-md"><span className={`h-2.5 w-2.5 rounded-full ${cameraStatus === 'connected' ? 'animate-pulse bg-error' : 'bg-outline'}`} /><span className="data-font text-sm text-white">{cameraStatus === 'connected' ? 'REC' : cameraStatus.toUpperCase()}</span><span className="mx-1 text-white/50">|</span><span className="data-font text-sm text-white">{time.toISOString().slice(11, 19)}:12</span></div><div ref={videoFrameRef} className="relative flex-1 overflow-hidden bg-surface-container-high"><video ref={videoRef} autoPlay muted playsInline className={`h-full w-full object-cover ${cameraStatus === 'connected' ? '' : 'hidden'}`} /><canvas className="pointer-events-none absolute inset-0 z-10" /><div className={`h-full w-full bg-cover bg-center opacity-60 ${cameraStatus === 'connected' ? 'hidden' : ''}`} style={{ backgroundImage: `url('${thermalUrl}')` }} /><div className="pointer-events-none absolute inset-0 flex items-center justify-center"><div className="flex h-[30%] w-[30%] items-center justify-center rounded-full border border-white/20"><div className="h-2 w-2 rounded-full bg-white/50" /><div className="absolute h-px w-full bg-white/20" /><div className="absolute h-full w-px bg-white/20" /></div></div></div><div className="absolute bottom-0 flex w-full items-center justify-between border-t border-white/10 bg-surface-container/90 p-3 backdrop-blur-md"><div className="flex gap-2"><button onClick={toggleCamera} className="rounded-md border border-white/5 bg-surface-variant p-2 text-on-surface hover:bg-surface-bright"><Icon>videocam</Icon></button><button onClick={() => setAiActive((value) => !value)} className={`flex items-center gap-2 rounded-md border px-4 py-2 ${aiActive ? 'border-secondary/30 bg-secondary/20 text-secondary' : 'border-white/10 bg-surface-variant text-on-surface-variant'}`}><Icon className="text-[20px]">psychology</Icon><span className="font-label-caps text-[10px]">AI DETECT</span></button></div><span className="data-font text-xs text-on-surface-variant">CAMERA: {cameraStatus.toUpperCase()} · PEOPLE: {detections.length}</span></div></div>
